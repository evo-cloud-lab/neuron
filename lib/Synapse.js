...
var StreamConnection = Class(process.EventEmitter, {
    ...
    /** @function
     * @description Abandon this connection
     */
    discard: function () {
    ...
    onError: function (err) {
        this.ignoreErrors || this.emit('error', err);
    },
...
var Synapse = Class(process.EventEmitter, {
    ...
    /** @field
     * @description Flag indicating busy sending messages
     */
    get busy () {
        return this._sendBusy;
    },
    ...
    /** @function
     * @description Abandon this Synapse
     */
    discard: function () {
        this.ignoreErrors = true;
        this.removeAllListeners();
...
var Connector = Class(Synapse, {
    ...
    constructor: function (connectUri, opts) {
        ...
        this.opts = {
            ...
            reconnectMax: 5,        // maximum reconnect attempts
    ...
    connect: function (connectUri, opts) {
        ...
        if (this._uri) {
            this.opts.reconnectMax > 0 && (this._attempts ++);
            process.nextTick(function () {
    ...
    onClose: function (hadError) {
        ...
        if (!this._disconnected && this.opts.reconnect &&
           (this.opts.reconnectMax < 0 || this._attempts < this.opts.reconnectMax)) {
    ...
    _parseConnectUri: function (connectUri, opts) {
        if (connectUri) {
            ...
            if (opts) {
                ...
                var attempts = parseInt(opts.reconnectMax);
                if (!isNaN(attempts)) {
                    this.opts.reconnectMax = attempts;
                }
                if (opts.reconnect != undefined) {
                    this.opts.reconnect = !!opts.reconnect;
                }
                var queueMax = parseInt(opts.sendQueueMax);
                if (!isNaN(queueMax)) {
                    this.opts.sendQueueMax = queueMax;
                }
            }
            this._attempts = 0;
        }
