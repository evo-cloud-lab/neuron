...
var LinkConnection = Class(Node, {
    constructor: function (conn, protocol, masterState) {
        ...
        this.trace = Trace('neuron:master:link', function () {
            return '<' + masterState.topology.id + '> ' + (this.id ? ':<' + this.id + '> ' : '');
        }.bind(this));
        this.trace.verbose('ACCEPT %s', conn.remoteAddress);
    ...
    send: function (event, data) {
        if (this.connection) {
            ...
            this.trace.debug('SEND %s: %j', this.id, msg);
    ...
    onTimeout: function () {
        this.trace.verbose('TIMEOUT %s', this.connection.remoteAddress);
        this.connection.close();
    },
    
    onClose: function () {
        this._cancelTimer();
        this.trace.verbose('CLOSE %s %s', this.connection.remoteAddress, this.id);
        if (this.valid) {
    ...
    onMessage: function (rawMsg) {
        ...
        this.trace.debug('MSG %s: %j', this.id ? this.id : '', msg);
