...
var State = Class({
    constructor: function (name, connector) {
        ...
        this.logger = new Logger('neuron:conn:' + name, '<' + this.topology.id + '> ');
    },
...
var MasterState = Class(State, {
    ...
    enter: function () {
        ...
        this.logger.notice('Master mode started: %j', this.topology.localNode.toObject());
        return this;
    },
    ...
    onConnection: function (link) {
        this.logger.verbose('CONNECT %s, %s:%d', link.id, link.address, link.port);
    ...
    onDisconnect: function (link) {
        this.logger.verbose('DISCONNECT %s, %s:%d', link.id, link.address, link.port);
    ...
    onMessage: function (msg, link) {
        this.logger.debug('MSG (%s): %j', link.id, msg);
    ...
    _syncTopology: function () {
        ...
        this.logger.debug('SYNC %j', updates);
        if (updates) {
            var fullTopology;
            this.logger.debug('SYNC %j', updates);
...
var ConnectedState = Class(State, {
    ...
    linkRequest: function (request) {
        request.reject(302, 'Not Master', { Location: this.masterLink.node.linkUri });
    },
    ...
    onLinkDown: function () {
        this.logger.verbose('MASTER LINK DOWN');
    ...
    _updateTopology: function (updates) {
        if (!updates.baseRevision || !updates.revision ||
            ...
            this.logger.warn('Invalid topology.update message: %j', updates);
            return;
        }
        if (updates.baseRevision != this.topology.revision) {
            this.logger.debug('Topology out-of-date: %d vs %d (base)', this.topology.revision, updates.baseRevision);
            ...
            if (!this.topology.valid) {
                // TODO
                this.logger.error('MASTER GONE: updates: %j', updates);
            }
        }
    }
});
...
var ConnectState = Class(State, {
    ...
    enter: function () {
        this.updateState();
        this.logger.verbose('CONNECT %s, %s:%d', this.topology.master.id, this.topology.master.address, this.topology.master.port);
...
var BootState = Class(State, {
    ...
    _connect: function () {
        ...
        this.logger.verbose('%s: CONNECT', bootstrap);
        request({
    ...
    _handleResponse: function (err, response, body) {
        var bootstrap = this.bootstraps[this.bootIndex];
        if (err) {
            this.logger.error('%s: ERROR: %s', bootstrap, err.message);
        } else if (response.statusCode >= 300) {
            this.logger.error('%s: FAIL %d: %j', bootstrap, response.statueCode, body);
        } else {
            this.logger.verbose('%s: BOOTED: %j', bootstrap, body);
            try {
                this.connector.from(this).transit(function () {
                ...
                this.logger.error('%s: ERROR: %s', bootstrap, e.message);
            }
        }
        this.bootIndex ++;
        if (this.bootIndex < this.bootstraps.length) {
            ...
            this.logger.error('No bootstrap available, run as master!');
            this.connector.from(this).transit(function () {
                return new MasterState(this.connector);
            }.bind(this));
        }
...
var Connector = Class(States, {
    constructor: function (opts) {
        ...
        this.opts = opts;
        this.logger = new Logger('neuron:conn', function () {
    ...
    start: function (callback) {
        ...
        this.server.listen(port, function () {
            ...
            this.topology = new Topology(this.opts.name, {
                id: this.opts.id ? this.opts.id : (this.opts.address + ':' + this.opts.port),
                address: this.opts.address,
                port: this.opts.port            
            }, this);
            this.logger.notice('Connector is listening on %s:%d', address.address, this.opts.port);            
            process.nextTick(function () {
