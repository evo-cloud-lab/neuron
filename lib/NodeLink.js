...
var NodeLink = Class(process.EventEmitter, {
    constructor: function (node, connector) {
        this.node = node;
        ...
        this.logger = new Logger('neuron:node:link', '<' + connector.topology.id + '> :' + node.id + ' ');
    },
    ...
    send: function (event, data) {
        if (this.connection) {
            ...
            this.logger.debug('SEND %s: %j', this.node.linkUri, msg);
    ...
    _connect: function () {
        this.logger.verbose('CONNECT %s: %s', this.node.linkUri, this.protocol.id);
    ...
    _reconnect: function (nodelay) {
        ...
        if (this.reconnect >= 0 && this.reconnect < RETRY_DELAY.length) {
            if (!nodelay) {
                this.logger.debug('DELAY %s: %d', this.node.linkUri, RETRY_DELAY[this.reconnect]);
            ...
            this.logger.verbose('TIMEOUT %s', this.node.linkUri);
    ...
    onConnect: function (conn) {
        ...
        this.logger.verbose('UP %s', this.node.linkUri);
        this.emit('up', this);
        return this;
    },
    
    onConnectFailed: function (err) {
        this.logger.error('CONNECT FAILED %s: %j', this.node.linkUri, err);
        this._reconnect();
    },
    ...
    onMessage: function (rawMsg) {
        ...
        if (msg) {
            this.logger.debug('MSG %s: %j', this.node.linkUri, msg);
            this.emit('message', msg, this);
        }
    },
    
    onConnectionError: function (err) {
        this.logger.error('ERROR %s: %s', this.node.linkUri, err.message);
        this.emit('error', err);
    },
    
    onConnectionClose: function (reasonCode, description) {
        this.logger.verbose('DOWN %s: %d %s', this.node.linkUri, reasonCode, description);
