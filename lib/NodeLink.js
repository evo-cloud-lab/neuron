...
var NodeLink = Class(process.EventEmitter, {
    constructor: function (node, connector) {
        ...
        this.trace = Trace('neuron:node:link', '<' + connector.topology.id + '> :' + node.id + ' ');
    },
    ...
    send: function (event, data) {
        if (this.connection) {
            ...
            this.trace.debug('SEND %s: %j', this.node.linkUri, msg);
    ...
    _connect: function () {
        this.trace.verbose('CONNECT %s: %s', this.node.linkUri, this.protocol.id);
    ...
    _reconnect: function (nodelay) {
        ...
        if (this.reconnect >= 0 && this.reconnect < RETRY_DELAY.length) {
            if (!nodelay) {
                this.trace.debug('DELAY %s: %d', this.node.linkUri, RETRY_DELAY[this.reconnect]);
            ...
            this.trace.verbose('TIMEOUT %s', this.node.linkUri);
    ...
    onConnect: function (conn) {
        ...
        this.trace.verbose('UP %s', this.node.linkUri);
        this.emit('up', this);
        return this;
    },
    
    onConnectFailed: function (err) {
        this.trace.error('CONNECT FAILED %s: %s', this.node.linkUri, err);
        this._reconnect();
    },
    ...
    onMessage: function (rawMsg) {
        ...
        if (msg) {
            this.trace.debug('MSG %s: %j', this.node.linkUri, msg);
            this.emit('message', msg, this);
        }
    },
    
    onConnectionError: function (err) {
        this.trace.error('ERROR %s: %s', this.node.linkUri, err);
        this.emit('error', err);
    },
    
    onConnectionClose: function (reasonCode, description) {
        this.trace.verbose('DOWN %s: %d %s', this.node.linkUri, reasonCode, description);
